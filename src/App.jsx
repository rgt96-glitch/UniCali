                ...
                <div style={{padding:isMobile?"0.75rem":"1rem 1.5rem 1.5rem",background:t.bg2,display:"grid",gridTemplateColumns:isMobile?"1fr":"repeat(auto-fill,minmax(220px,1fr))",gap:10}}>
                  {works.map(w=>(
                    <div key={w.id} style={{border:`1px solid ${t.border}`,borderRadius:2,padding:"0.875rem",background:t.bg3}}>
                      ...
                      <p style={{fontSize:12,color:t.muted,lineHeight:1.6,margin:"0 0 8px"}}>{w.description}</p>
                      <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
                        {w.tags.map(tag=><span key={tag} style={{fontSize:9,color:t.tag,border:`1px solid ${t.border}`,padding:"2px 5px",borderRadius:1,letterSpacing:"0.06em",textTransform:"uppercase"}}>{tag}</span>)}
                      </div>
                    </div>
                  ))}
                </div>
